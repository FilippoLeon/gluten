...
int main() {
    ...
    gluten::shader::VertexShader vs(app.GlutenRoot("/shader/examples/lighting.vs"));
    gluten::shader::FragmentShader fs(app.GlutenRoot("/shader/examples/lighting.fs"));
    gluten::shader::ShaderProgram shader(vs, fs);

    gluten::geometry::Mesh plane = gluten::geometry::Plane({ 0, 0, 1 });
