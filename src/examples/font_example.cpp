...
int main() {
    ...
    gluten::shader::VertexShader text_vs({ "TEXTURE" }, app.GlutenRoot("/shader/examples/lighting.vs"));
    gluten::shader::FragmentShader text_fs({ "TEXTURE" }, app.GlutenRoot("/shader/examples/lighting.fs"));
    gluten::shader::ShaderProgram text_shader(text_vs, text_fs);
    gluten::fonts::Font("Roboto-Black.ttf", text_shader);
